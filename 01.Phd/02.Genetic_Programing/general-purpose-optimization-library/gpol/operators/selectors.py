...
def prm_dernd_selection(n_sols):
    """ Implements random selection algorithm (for DE)

    This function is used to provide the rnd_selection (inner function)
    with the necessary environment (the outer scope) - the number of
    solutions to select at random. This kind of selection is typically
    used in DE where different the mutation strategies make use of
    different number of randomly selected candidate solutions.
    
    Parameters
    ----------
    n_sols : int
        Number of randomly selected representations to return.

    Returns
    -------
    dernd_selection : function
        A function which implements random selection algorithm that
        returns n_sols indices.
    """
    def dernd_selection(pop):
        """ Implements random selection algorithm

        Typically used with DifferentialEvolution, this function allows
        to generate a user-specified number of random indices for the
        sake of mutation.
        ...
        -------
        torch.Tensor
            A tensor containing random indices in [0, len(pop)[ range.
        """
        return torch.randint(low=0, high=len(pop.repr_), size=(n_sols,))

    return dernd_selection
