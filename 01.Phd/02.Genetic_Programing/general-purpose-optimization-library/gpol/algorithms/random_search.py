...
from gpol.algorithms.search_algorithm import SearchAlgorithm
...
class RandomSearch(SearchAlgorithm):
    ...
    def solve(self, n_iter=20, tol=None, n_iter_tol=5, start_at=None, test_elite=False, verbose=0, log=0):
        ...
        if log > 0:
            # Evaluates the best-so-far solution on the test partition, if necessary
            if test_elite:
        ...
        for it in range(1, n_iter + 1):
            ...
            if rand_cand_sol.valid:
                ...
                # Optionally, evaluates the solution on on the test partition
                if test_elite:
                    self.pi.evaluate_sol(self.best_sol, test_elite)
