...
bounded knapsack combinatorial optimization problem. The procedure
...
def main():
    ...
    device = "cpu"
    # Characterizes the bounded Knapsack problem
    capacity, n_items, max_rep, min_ = 1000, 100, 4, False
    # Defines the lower and the upper bounds for weights' and values' generation
    lb, ub = 0, 10
    # Defines the bounds
    bounds = torch.tensor([0, max_rep]).to(device)
    ...
    rs_pars = {"initializer": prm_rnd_vint(0, max_rep + 1)}
    # Defines HC's parameters
    p_m_bound = 0.2  # probability of adding/removing a given number of items at a given index in the chromosome
    mutator = prm_rnd_int_ibound(p_m_bound, 0, max_rep + 1)
    hc_pars = {"initializer": prm_rnd_vint(0, max_rep + 1), "nh_function": mutator, "nh_size": pop_size}
    # Defines SA's parameters
    control, update_rate = 1, 0.9
    sa_pars = {"initializer": prm_rnd_vint(0, max_rep + 1), "nh_function": mutator, "control": control,
               "update_rate": update_rate, "nh_size": pop_size}
    # Defines GA's parameters
    p_m = 0.3
    ga_pars = {"initializer": prm_rnd_mint(0, max_rep + 1), "selector": prm_tournament(pressure=0.08),
               "mutator": mutator, "crossover": one_point_xo, "p_m": p_m, "p_c": 1-p_m, "pop_size": pop_size,
               "elitism": True, "reproduction": False}

    # ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    # > > > Setup the logging properties
    # Creates the logger
    experiment_label = "KnapsackBounded"
    ...
    for seed in range(n_runs):
        ...
        sspace = {"capacity": capacity, "n_dims": n_items,
                  "weights": torch.FloatTensor(n_items).uniform_(lb, ub).to(device),
                  "values": torch.FloatTensor(n_items).uniform_(lb, ub).to(device), "bounds": bounds}
        # Creates a problem's instance (PI)
        pi = KnapsackBounded(sspace, torch.matmul, min_)
        ...
        for isa_name, isa in isas.items():
            ...
            # Solves the PI
            isa.solve(n_iter=n_iter_, verbose=2, log=2 if isa_name != "RS" else 1)
